...
export const handler = async (event) => {
    ...
    const ReserveBench = (rest_uuid) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Benches WHERE restUUID = ? AND isReserved = 0 LIMIT 1", [rest_uuid], (error, rows) => {
                if (error) {
                    return reject(error);
                }
    ...
    try {
        ...
        const rows = await ReserveBench(rest_uuid);
